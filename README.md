...
> 가벼운 all-rounder CLI 코딩 에이전트 — 전용 터미널 UI, OpenAI-compatible 모델 지원, 온디맨드 스킬·MCP·세션 이어가기.

allCode는 에이전트형 코딩 도우미의 핵심 구조는 유지하되, 거대한 단일 파일 구조를
피하기 위해 provider-neutral model event, core contract, routing/policy, tool
execution, workspace context, memory, generation workflow, TUI/headless
entrypoint를 각각 별도 패키지로 나누어 구현합니다.

기본 실행 경로(headless·TUI)는 설정된 OpenAI-compatible 모델과 바로 통신하고,
...
- [주요 기능](#주요-기능)
- [설치](#설치)
  - [방법 A — 사전 빌드 바이너리 (Python 불필요)](#방법-a--사전-빌드-바이너리-python-불필요)
  - [방법 B — pip / 소스 설치](#방법-b--pip--소스-설치)
- [바이너리 패키징 (빌드 · 릴리스)](#바이너리-패키징-빌드--릴리스)
- [모델 설정](#모델-설정)
- [실행 방법](#실행-방법)
- [슬래시 명령어](#슬래시-명령어)
- [스킬 (.allCode/skills)](#스킬-allcodeskills)
- [MCP 서버 관리](#mcp-서버-관리)
- [프로젝트 구조](#프로젝트-구조)
- [테스트](#테스트)
- [개발 원칙](#개발-원칙)
...
```bash
# 1) 설치 (둘 중 하나)
#    - 바이너리: 아래 "방법 A" 참고 (Python 불필요)
#    - 소스:
python -m pip install -e .

# 2) API 키 설정 (프로젝트 루트 .env 권장)
echo 'ALLCODE_API_KEY=sk-...' >> .env

# 3) 실행
allcode                                   # 터미널 UI
allcode --headless "이 저장소를 설명해줘"   # 헤드리스(1회 응답)
allcode --version                         # 버전 확인
allcode --check                           # 설정 진단(백엔드 접속 없이)
```

> 모델·base URL은 `--model` / `--base-url` 플래그나 `.allCode/config.yaml`로
> 지정합니다. 자세한 내용은 [모델 설정](#모델-설정)을 참고하세요.
...
- **사전 빌드 바이너리 배포**: PyInstaller onefile 바이너리로 Python 없이 설치.
  → [바이너리 패키징](#바이너리-패키징-빌드--릴리스)
...
allCode는 두 가지 방법으로 설치할 수 있습니다. 일반 사용자는 **방법 A(바이너리)**,
개발·기여자는 **방법 B(소스)**를 권장합니다.

### 방법 A — 사전 빌드 바이너리 (Python 불필요)

[GitHub Releases](https://github.com/kimtj319/allCode/releases)에서 플랫폼에 맞는
파일을 내려받아 압축을 풀고 실행합니다. Python·pip 설치가 필요 없습니다.

| 플랫폼 | 내려받을 파일 |
|---|---|
| macOS (Apple Silicon) | `allcode-macos-arm64.tar.gz` |
| macOS (Intel) | `allcode-macos-x86_64.tar.gz` |
...
curl -LO https://github.com/kimtj319/allCode/releases/latest/download/allcode-macos-arm64.tar.gz
tar -xzf allcode-macos-arm64.tar.gz
chmod +x allcode-macos-arm64
./allcode-macos-arm64 --version

# PATH에 등록해 'allcode'로 실행하려면
sudo mv allcode-macos-arm64 /usr/local/bin/allcode
allcode --version
```

Windows (PowerShell): `allcode-windows-x86_64.exe.zip`을 풀어 나온 실행 파일을
원하는 위치에 두고 실행합니다.

무결성 검증: 각 릴리스에는 `checksums.txt`가 함께 게시됩니다.

```bash
sha256sum -c checksums.txt        # 내려받은 파일과 같은 디렉터리에서
```

> 참고: Linux 바이너리는 glibc 기반 배포판을 대상으로 합니다(musl 기반 Alpine은
> 기본 바이너리로 지원하지 않습니다). Windows arm64는 빌드하지 않습니다.
...
개발·테스트용 optional dependency:

```bash
python -m pip install -e ".[dev]"
```

코드 분석용 tree-sitter 백엔드(선택):

```bash
python -m pip install -e ".[source-intelligence]"
```

`requirements.txt`는 `requirements.txt` 기반 설치를 기대하는 환경을 위한 호환
설치 파일입니다. 패키지 정의의 기준은 `pyproject.toml`입니다.
...
allCode는 [PyInstaller](https://pyinstaller.org) **onefile 바이너리**로 배포되어
최종 사용자가 Python/pip 없이 설치할 수 있습니다. 패키징 구성은 다음과 같습니다.

| 파일 | 역할 |
|---|---|
| `packaging/allcode.spec` | PyInstaller spec. 진입점은 `src/allCode/__main__.py`, `collect_submodules`로 지연 import를 포착하고, 설치되어 있으면 tree-sitter 문법/네이티브 라이브러리를 함께 번들 |
| `packaging/build_onefile.py` | `--name`으로 바이너리를 빌드해 `dist/`에 출력 |
| `.github/workflows/build-binaries.yml` | 5개 타깃 빌드 → `--version`/`--check` 스모크 → 태그 시 릴리스에 아카이브·`checksums.txt` 게시 |
| `pyproject.toml` | `package` extra(`pyinstaller`) |

자세한 절차는 [`PACKAGING.md`](PACKAGING.md)에 정리되어 있습니다.
...
python packaging/build_onefile.py --name allcode-macos-arm64   # dist/ 에 생성
```

빌드된 바이너리는 다음 스모크 테스트를 통과해야 합니다.

```bash
./dist/allcode-macos-arm64 --version    # -> allcode <version>
./dist/allcode-macos-arm64 --check      # 설정 진단, 백엔드 접속 없이 종료
```

### 릴리스 빌드

GitHub Actions가 macOS arm64/x86_64, Windows x86_64, Linux x86_64/arm64를 모두
빌드합니다. 수동(`workflow_dispatch`)으로 실행하거나, 버전 태그를 푸시하면 됩니다.
...
태그 빌드는 모든 플랫폼 바이너리와 `checksums.txt`를 GitHub 릴리스에 게시합니다.
...
| `ALLCODE_API_KEY` | API key 값. 설정 시 allCode는 `ALLCODE_API_KEY`를 key source로 사용 |
| `ALLCODE_API_KEY_ENV` | API key를 담고 있는 다른 환경 변수명 |
| `ALLCODE_WORKSPACE` | workspace root |
| `ALLCODE_APPROVAL_MODE` | `ask`, `auto`, `rules` |
| `ALLCODE_WEB_SEARCH_BACKEND` | `duckduckgo_html`(기본), `searxng`, `http_json`, `disabled` |
| `ALLCODE_WEB_SEARCH_URL` | web search endpoint (기본 `https://html.duckduckgo.com/html/`) |
| `ALLCODE_WEB_SEARCH_API_KEY_ENV` | web search token을 담은 환경 변수명 |
| `ALLCODE_WEB_SEARCH_TIMEOUT` | web search timeout(초) |
| `ALLCODE_WEB_SEARCH_LANGUAGE` | SearXNG backend 기본 언어 |

config 파일에는 API key 값을 저장하지 않고, API key가 들어 있는 환경 변수명만
저장합니다. 프로젝트 루트의 `.env` 파일도 자동으로 읽습니다. 이때 `ALLCODE_`로
시작하는 변수만 반영하며, 이미 셸에 설정된 환경 변수는 `.env` 값으로 덮어쓰지
않습니다. 따라서 repository root에서 실행할 때는 `.env`에 `ALLCODE_API_KEY=...`를
두는 것만으로 충분합니다. `source .env`를 쓸 경우 `export` 없이 정의한 변수는 셸
하위 프로세스에 전달되지 않을 수 있으므로, allCode의 자동 `.env` loader를 기준으로
생각하는 편이 안전합니다.
...
`model.context_window_tokens`를 실제 모델 윈도(예: 32768)로 설정하면, 긴 세션에서
대화가 윈도에 맞게 자동 압축되어 오버플로를 막고(작은 윈도) 더 많은 근거를
보존합니다(큰 윈도). `agent.system_prompt_append`에 어조·형식·상시 지침을 적으면
매 턴에 적용됩니다(캐시 친화적 정적 영역에 배치).
...
`pip install -e .`(또는 바이너리) 이후 `allcode` 명령을 사용할 수 있습니다.

```bash
allcode --help
allcode --version                     # 버전 출력
allcode --check                       # 설정 진단(백엔드 접속 없이 종료, --diagnose 별칭)
...
```bash
allcode                 # 기본 terminal-native UI
allcode --textual       # 선택형 Textual UI
allcode --plain-terminal
```

기본 `allcode`는 Textual fullscreen 앱이 아니라 terminal-native UI를 실행합니다.
본문 출력은 일반 터미널 scrollback을 사용하고, 하단 composer만 고정해 입력,
history, completion, paste placeholder, runtime status/spinner를 표시합니다.
Enter/LF는 제출이고, multiline 입력은 Alt+Enter를 사용합니다.

`allcode --textual`은 선택형 Textual UI입니다. Textual이 설치되지 않았으면
terminal-native UI로 fallback합니다. `--plain-terminal`은 기본 terminal-native
UI를 명시하는 compatibility alias입니다.
...
- `packaging`: PyInstaller spec과 onefile 빌드 스크립트. → [바이너리 패키징](#바이너리-패키징-빌드--릴리스)
...
<details>
<summary>Current Limitations 펼쳐 보기</summary>
...
<details>
<summary>최근 업데이트(refactor/unified-agent-loop 브랜치 기준) 펼쳐 보기</summary>

- **바이너리 패키징** — PyInstaller onefile 바이너리와 크로스플랫폼 GitHub Actions
  빌드/릴리스 워크플로우를 추가해 Python 없이 설치 가능. `--version`/`--check`
  스모크 테스트와 `checksums.txt` 게시 포함. → [바이너리 패키징](#바이너리-패키징-빌드--릴리스)
...
- **통합 에이전트 루프(unified loop)** — 라우팅을 advisory로 두고 모델이 전체
  ...
  통합 프롬프트로 제공.
- **TUI 개선** — 터미널 resize 시 전체 트랜스크립트 재렌더(계단 현상 수정), 턴 사이
  구분선, 다중모달 이미지 입력(`@image.png` 멘션), 동적 모델 표시 footer, 슬래시
  명령 결과 패널 타이틀.
