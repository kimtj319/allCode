...
### ✅ 완료 (테스트 동반, 커밋·푸시)
- **§2 데드코드**: `core/errors.py` 미사용 예외 5종, `core/events.py` 미발행 이벤트 7종 삭제, 고아 모듈 `tui/input_box.py` 삭제, ruff `F401` 6건 정리(phase_gate.py 재노출 import는 오탐 복원). (커밋 `5e6ac3d`)
- **§1 P0 (치명적 성능)**: repo-map 캐시 배선 — `RepoMapBuilder`가 mtime 기반 인메모리 메모 + `.allCode/repo_map_cache.json` 영속으로 **턴마다 ~1800파일 재파싱 제거**. 변경 파일만 재추출. (커밋 `f37b067`) 전체 테스트 시간 ~15s→~12s 부수 효과.

### ⏳ 남은 항목(후속, 우선순위 순)
- **§1 P1**: 시작 인덱스 지연/백그라운드 빌드 + ignore 디렉터리 순회 가지치기(`indexer.py`).
- **§1 P2**: `memory/store.py` 파일 읽기 mtime 캐시.
- **§1 P3**: 함수 내 정규식 모듈 레벨 승격(`validation_failure_parser.py` 등) — 낮은 효과.
- **§3 중복 헬퍼 추출**, **§4 과편화 병합**, **§5 갓 모듈 분리** — 구조 리팩터, behavior-neutral. 각 단위 커밋+pytest.

> 본 라운드는 가장 안전·고효과인 데드코드 정리 + P0 성능을 적용했다. 남은 항목은 동일 가드레일(전체 그린 유지)로 증분 진행한다.
