...
## 구현 결과 (2026-06-13, 5차 — 탐색 깊이: spine)

codex가 v8을 55%로 평가(개선됐으나 main→runtime→loop 최상위 경로·핵심 계약 누락,
import 추론). 원인: 대표 read가 패키지당 ~1파일 round-robin이라 entrypoint 체인이
누락.

수정:

- `source_overview_metadata.py`: 광역 분석 대표 read 예산 12→16.
- `source_overview.py`: index 선택이 entrypoint(main/runtime/cli/__main__)를 항상
  우선 포함(작은 루트 그룹이 size-cap에 밀려 빠지지 않게).
- `source_ranking.py`: 대표 read 선택이 `PRIORITY_ENTRYPOINTS`(main/__main__/cli/
  runtime)를 그룹 랭킹보다 먼저 포함. `runtime`을 entrypoint로 인식.
- `source_overview_roles.py`: `result`를 아키텍처 stem에 추가.

검증(실모델 v10): 실행 흐름이 실제 spine을 추적 — `__main__.py`→`main.py`(argparse·
ConfigManager)→`runtime.py`(UI/headless 선택)→`tui/runtime.py` run_interactive_session
(terminal-native UI 정확)→`round_runner`→PromptBuilder·ModelRouter·LLMClient·
phase_gate·final_answer_context. codex 지적 #1(main→runtime 경로)·#5(terminal UI)
직접 해소. 전체 775 passed.
