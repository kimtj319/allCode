...
- `00`~`12`는 기본 구현 대상 문서이고, `15`는 Codex 수준 TUI 보강 구현 문서다. `16`은 실제 Codex 기본 실행 관찰을 반영한 terminal-native UI 정정 문서다. `17`은 실제 tool 사용 검증 이후의 model-routed tool system 보강 문서다. `18`은 공개 agent pattern 기반 hardening 문서다. `19`는 실제 모델 stress의 남은 completion gap을 닫기 위한 후속 보강 문서다. `20`은 agy 토론과 최신 stress 결과를 반영한 harness-agent completion 보강 문서다. `21`은 오픈소스 에이전트 대비 95% 완성도 목표를 위한 strict gate, large-file context, loop SRP, diagnostic 보강 문서다. `26`은 남은 open-source parity gap을 닫기 위한 requested artifact checklist, validation repair state, patch strategy memory 보강 문서다. `27`은 Plan 26 실모델 회귀 결과를 바탕으로 validation repair target, patch ambiguity strategy, test-authoring escalation을 보강하는 문서다. `13`~`14`는 검토 이력 부록이다. 구현 중 판단이 충돌하면 부록보다 `00`~`12`, `15`, `16`, `17`, `18`, `19`, `20`, `21`, `26`, `27`의 최신 계약을 우선한다.
...
| 26 | `26_remaining_open_source_parity_hardening_plan.md` | 남은 fail/warning을 닫기 위한 requested artifact checklist, validation repair state, patch strategy memory 보강 |
| 27 | `27_validation_repair_convergence_plan.md` | Plan 26 실모델 회귀를 보완하기 위한 validation repair target, patch ambiguity strategy, test-authoring escalation 보강 |
...
5. 요청 5: `10`, `11`, `12`, `15`, `16`을 기준으로 TUI, quality test, Codex 수준 persistent composer, terminal-native 기본 UI, end-to-end 검증을 구현한다.
...
- `26_remaining_open_source_parity_hardening_plan.md`: 남은 open-source parity gap을 닫기 위한 requested artifact checklist, validation repair state, patch strategy memory 보강 계획.
- `27_validation_repair_convergence_plan.md`: Plan 26 실모델 회귀를 바탕으로 validation repair target, patch ambiguity strategy, test-authoring escalation을 구체화한 보강 계획.
