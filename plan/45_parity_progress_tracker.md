...
| TUI/interactive UX | 94-95% | Table per-column rules/row separators, finer block vertical rhythm, dot-style spinner glyph | `plan/58`: fixed scroll-region clobber + gap + streamed block fragmentation, then matched Codex turn marker (`•`+2-col indent), `›` prompt glyph, header `━` table rule, removed composer rule. Side-by-side PTY capture of real Codex CLI vs allCode (same prompts) judged ~94-95% with no UI breakage |
