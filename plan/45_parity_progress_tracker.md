...
| TUI/interactive UX | 96-97% | Dot-style spinner glyph, plain vs syntax-highlighted code blocks, minor table column padding/streaming block rhythm | `plan/58`: fixed scroll-region clobber + gap + streamed block fragmentation; matched Codex turn marker (`•`+2-col indent), `›` prompt glyph, segmented `━` header + per-row `─` table rules, block vertical rhythm, removed composer rule, context footer. Side-by-side PTY capture of real Codex CLI vs allCode (same prompts) judged ~96-97% with no UI breakage |
