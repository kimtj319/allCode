...
- `python -m pytest` → 772 passed, 3 skipped(마커/`›` 변경으로 TTY 테스트 3건
  업데이트).
- codex(read-only) 평가 추이: 80-83% → 90-92% → 94-95% → **96-97%**,
  "진짜 UI 깨짐 없음".

## Phase 5 (표 규칙·블록 리듬 정제, 2026-06-13)

- 표를 custom Rich Box로 렌더: 분절된 heavy `━` 헤더 규칙 + 행마다 light `─`
  구분선, 세로/외곽 보더 없음(실제 Codex 표와 구조 일치).
- 단일 렌더 호출 내 블록(제목/문단/목록/코드) 사이 빈 줄로 수직 리듬 복원.
  스트리밍 청크 경계에서는 보수적으로 유지해 문단/목록/소프트 개행이 잘못된
  빈 줄로 쪼개지지 않도록 함.
- 재평가: **96-97%** (95% 목표 초과). 남은 차이는 모두 낮은 우선순위:
  dot-style 스피너(실제 Codex 프레임 미상), 코드 블록 구문강조(allCode 의도적
  향상), 표 패딩/스트리밍 블록 리듬 미세차.
