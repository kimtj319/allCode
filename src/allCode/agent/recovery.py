...
class ToolLoopGuard:
    ...
    def __init__(self, *, window_size: int = 10, repeat_limit: int = 3, recent_span: int = 5) -> None:
    ...
    def record(self, tool_call: ToolCall) -> tuple[ToolLoopSignature, bool]:
        ...
        detected = recent.count(key) >= self._repeat_limit
...
def needs_validation_repair(routing, evidence: CompletionEvidence) -> bool:
    return bool(routing.requires_validation and evidence.has_file_change() and evidence.validation_passed is not True)
