...
class SlashCommandHandler:
    ...
    async def handle(self, command: str) -> SlashCommandResult:
        ...
        if normalized.split(maxsplit=1)[0] in {"/tools", "/model", "/approval", "/plan", "/permissions", "/thinking", "/mcp", "/skills", "/resume", "/config", "/init", "/doctor", "/export", "/pr", "/agents", "/status", "/debug"}:
        ...
        if custom_name in self.custom_commands:
            ...
            return SlashCommandResult(submit_prompt=expand_command(template, args, cwd=self.workspace_root))
