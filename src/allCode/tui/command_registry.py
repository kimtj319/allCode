...
def default_commands() -> list[CommandSpec]:
    return [
        ...
        CommandSpec(name="/undo", description="Undo allCode's last git auto-commit (history-level; use after a committed turn).", usage="/undo"),
        CommandSpec(name="/rewind", description="Restore files to the last turn's checkpoint (working-tree level; reverts even uncommitted edits).", usage="/rewind"),
        CommandSpec(name="/resume", description="List recent sessions, or load one's history into this session (/resume <id|name>).", usage="/resume [<id|name>]", options=["list"]),
